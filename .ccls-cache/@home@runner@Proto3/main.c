...
/*=================================================================================*/
void separate(){ //Output formatting, separates questions
  printf("\n\33[1;32m=-=-=-=-=--=-=-=-=-=-=-=-=--=-=-=-=-=-=-=--=-=-=-=\033[0m\n");
}

void colour(char *text, char *colourcode){ //Output formatting, change text colour 
...
/*=================================================================================*/
    ...
    { 
        ...
        //Copy each field into data array
        for(int w = 0;w<countStr;w++) 
        { 
            int difference=0; 
            if(w==0) 
            { 
...
/*=================================================================================*/
  ...
  for(int e = 0; e < 5; e++) {
    ...
    for(int yearIndex = 0; yearIndex < 7; yearIndex++) {
      ...
      // loop through the rows for each year and gender
...
//2 
  ...
  printf("The Province with the highest percentage of Diabetes is: %s, with a rate of %.2lf%%\n", highestDiabetesProvince, HDR);
  
  printf("The Province with the lowest percentage of Diabetes is: %s, with a rate of %.2lf%%\n", lowestDiabetesProvince, LDR);


  separate();
/*=================================================================================*/
  
//3 - Needs to be fixed, error output
  char *q3 = "\nQuestion 3)\n\n";
  colour(q3, "\033[33m");
  
  int higherCountryPtr[4]; 
  int lowerCountryPtr[4]; 
  int higherCnt=0; 
  int lowerCnt = 0; 
  ...
  printf("The Provinces With Diabetes Averages Lower Than The National Average:\n\n"); 
  for(int i=0;i<lowerCnt;i++) { 
    printf("%s, with a rate of %.2lf%%\n",File[GenderRow_Start[lowerCountryPtr[i]][0][0]][1],totalDiabetesAvg[lowerCountryPtr [i]]); 
  } 
  printf("\nThe Provinces With Diabetes Averages Higher Than The National Average:\n\n"); 
...
int HighestYrPtr = 0;
int LowestYrPtr = 0;
int ProvHighestYrPtr = 0;
int ProvLowestYrPtr = 0;
int ProvIndex = 0;
int yearIndex = 0;

// Find the index of the province and year with the highest and lowest diabetes rates
  while (ProvIndex < 5) {
      yearIndex = 0;
      while (yearIndex < 7) {
